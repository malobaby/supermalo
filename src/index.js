...
export async function runInstall(options = {}) {
  ...
  console.log('');
  console.log(chalk.cyan('将要安装 Supermalo 的所有组件：'));
  ...
  if (!options.yes) {
    const confirmed = await promptConfirmation('确认安装 Supermalo？');
    if (!confirmed) {
...
export async function runUpdate(options = {}) {
  ...
  if (!options.yes) {
    const confirmed = await promptConfirmation('确认更新 Supermalo？');
    if (!confirmed) {
