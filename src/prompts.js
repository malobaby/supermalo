...
export function showBanner() {
  ...
  console.log(chalk.cyan.bold('  ║                                           ║'));
  console.log(chalk.cyan.bold('  ║') + chalk.white.bold(`           Supermalo（v${version}）                 `) + chalk.cyan.bold('║'));
  ...
  const { confirmed } = await inquirer.prompt([
    {
      type: 'confirm',
      name: 'confirmed',
      message: chalk.yellow('确定要卸载 Supermalo 吗？'),
      default: false
    }
  ]);
...
export async function promptMainMenu() {
  ...
  const choices = [
    { name: '安装 Supermalo', value: 'install' },
    { name: '更新 Supermalo', value: 'update', disabled: !installedVersion ? '（未安装）' : false },
    { name: '卸载 Supermalo', value: 'uninstall', disabled: !installedVersion ? '（未安装）' : false },
