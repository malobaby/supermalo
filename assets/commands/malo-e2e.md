...
此命令调用 **malo-e2e-runner** 代理来使用 Playwright 生成、维护和执行端到端测试。
...
- 使用 `/malo:malo-plan` 识别需要测试的关键流程
- 使用 `/malo:malo-tdd` 进行单元测试（更快、更细粒度）
- 使用 `/malo:malo-e2e` 进行集成和用户流程测试
- 使用 `/malo:malo-code-review` 验证测试质量

## 相关代理

此命令调用位于以下位置的 `malo-e2e-runner` 代理：
`~/.claude/agents/malo-e2e-runner.md`
