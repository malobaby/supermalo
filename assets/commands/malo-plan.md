...
此命令调用 **malo-planner** 代理，在编写任何代码之前创建全面的实施计划。
...
```
用户：/malo:malo-plan 我需要在市场结算时添加实时通知功能
...
**关键**：malo-planner 代理在您明确确认计划（使用 "是"、"继续" 或类似肯定回复）之前**不会**编写任何代码。
...
计划完成后：
- 使用 `/malo:malo-tdd` 通过测试驱动开发实施
- 如果出现构建错误，使用 `/malo:malo-build-fix`
- 使用 `/malo:malo-code-review` 审查已完成的实施

## 相关代理

此命令调用位于以下位置的 `malo-planner` 代理：
